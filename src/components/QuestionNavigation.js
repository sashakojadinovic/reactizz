...
function QuestionNavigation(props) {
    ...
    return (
        <div className="row question-nav">
            ...
            <button disabled={props.score?true:''} onClick={() => setModal(true)} className={`btn finish-btn ${props.score ? "green score-btn" : "yellow"}`} >{props.score ? props.score : <Clock stopped={props.stopped} />}</button>
